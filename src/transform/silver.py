...
def create_silver_layer(force_refresh: bool = False, **kwargs) -> None:
    """
    Cria a camada Silver: limpeza, padronização, validação e upload para OCI.
    """
    try:
        ...
        if not (SILVER_FILE.exists() and not force_refresh):
            ...
            logger.success("[SILVER] Camada Silver criada com sucesso.")
        else:
            logger.info("[SILVER] Arquivo Silver já existe. Pulando processamento.")
